...
async def test_create_and_read(async_client: AsyncClient):
    type(async_client)
    import inspect
    print(inspect.getmembers(async_client, predicate=inspect.ismethod))
...
async def test_done_flag(async_client):
    ...
    # /tasks/{id}/doneにPUTしてタスクを完了
    response = await async_client.put("/tasks/1/done")
    assert response.status_code == starlette.status.HTTP_200_OK

    # /tasks/{id}/doneにGETしてタスクの完了フラグを400で取得
    response = await async_client.put("/tasks/1/done")
    assert response.status_code == starlette.status.HTTP_400_BAD_REQUEST

    # /tasks/{id}/doneにDELETEしてタスクの完了フラグを削除
    response = await async_client.delete("/tasks/1/done")
    assert response.status_code == starlette.status.HTTP_200_OK

    # /tasks/{id}/doneにGETしてタスクの完了フラグを404で取得
    response = await async_client.delete("/tasks/1/done")
